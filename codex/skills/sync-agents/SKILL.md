...
# Sync Agents Skill (Codex)

Metodología para mantener simetría estructural entre `opencode/` y `codex/` dentro del repo de dotfiles `~/agent-dotfiles/`. **NO aplica a ningún otro repo.**

Este skill es el espejo Codex del `opencode/skills/sync-agents/SKILL.md`. Las reglas son las mismas; las diferencias operativas son:

- Codex no tiene comandos slash personalizados; toda la UX pasa por NL. La palabra clave canónica es `apply` (modo destructivo) o ausencia de args (modo `dry-run`).
- Codex ejecuta bash; los comandos de inspección usan rutas `/c/...` estilo Git Bash.
- Codex no tiene `description:` en frontmatter de skills que requiera `description:` estricto; el frontmatter aquí es declarativo pero no se consume para tool routing.

## Cuándo disparar

Cargar este skill cuando el usuario exprese intención de sincronización cross-agent:
...
- Cualquier mención explícita de `sync-agents` o de las palabras canónicas.

**NO usar este skill** para:

- Cualquier repositorio distinto a `~/agent-dotfiles/`.
- Operaciones de git (`commit`, `push`) — prohibidas sin instrucción explícita.
- Symlinks de dotfiles — usar el skill `dotfiles`.
- Cambios al `AGENTS.md` del proyecto actual (no del repo dotfiles).
- Compilar, build o tests — prohibido por `codex/AGENTS.md`.
...
1. Imprimir side-by-side (diff resumido si los archivos son >100 líneas; `diff -u` por defecto).
...
2. **De-Codex paths** — paths tipo `$CODEX_HOME/skills/...` o `~/.codex/...` se reescriben a `$DOTFILES_HOME/opencode/...` o `~/agent-dotfiles/opencode/...`. Solo aplica en dirección CX → OC.
...
8. **Sin ejecución de git**: este skill NO corre `git add`, `git commit`, ni `git push`. El usuario lo hace por separado.
9. **Sin ejecución de `install.ps1`**: este skill NO toca symlinks. Para eso existe el skill `dotfiles`.
...
## Equivalencia con el comando `/sync-agents`

El comando slash OpenCode `/sync-agents` delega toda esta metodología. Codex no tiene comando slash; la misma funcionalidad se invoca por NL ("aplica sync-agents", "qué difiere entre lados", etc.) y este skill es el contrato de comportamiento.
